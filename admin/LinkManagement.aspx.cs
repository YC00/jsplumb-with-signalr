...
{
    public partial class LinkManagement : System.Web.UI.Page
    {
        ...
        protected void tvGroup_SelectedNodeChanged(object sender, EventArgs e)
        {
            ...
            string strQuery = "select TemplateID, TemplateColor" +
                                " from ConceptMap_SelectedLinkTemplate " +
                                "where " +
                                "ORCSGroupID='" + hfNodeValue.Value + "'";
        ...
        private void SetInitialRow()
        {
            ...
            ViewState["CurrentLinkTable"] = dt;
            ViewState["CurrentLinkTable2"] = dt2;
        ...
        private void AddNewRowToGrid()
        {
            ...
            if (ViewState["CurrentLinkTable"] != null)
            {
                DataTable dtCurrentTable = (DataTable)ViewState["CurrentLinkTable"];
                ...
                if (dtCurrentTable.Rows.Count > 0)
                {
                    ...
                    ViewState["CurrentLinkTable"] = dtCurrentTable;
        ...
        private void AddNewRowToGrid2()
        {
            ...
            if (ViewState["CurrentLinkTable2"] != null)
            {
                DataTable dtCurrentTable = (DataTable)ViewState["CurrentLinkTable2"];
                DataRow drCurrentRow = null;
                if (dtCurrentTable.Rows.Count > 0)
                {
                    for (int i = 1; i <= dtCurrentTable.Rows.Count; i++)
                    ...
                    dtCurrentTable.Rows.Add(drCurrentRow);
                    ViewState["CurrentLinkTable2"] = dtCurrentTable;
        ...
        private void SetPreviousData()
        {
            ...
            if (ViewState["CurrentLinkTable"] != null)
            {
                DataTable dt = (DataTable)ViewState["CurrentLinkTable"];
                if (dt.Rows.Count > 0)
                {
        ...
        private void SetPreviousData2()
        {
            ...
            if (ViewState["CurrentLinkTable2"] != null)
            {
                DataTable dt = (DataTable)ViewState["CurrentLinkTable2"];
                if (dt.Rows.Count > 0)
                {
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
            ...
            try
            {
                using (SqlConnection conn = new SqlConnection(strConnString))
                {
                    using (SqlCommand comm = new SqlCommand())
                    {
                        ...
                        conn.Open();
                        StrQuery = @"INSERT INTO ConceptMap_LinkTemplate(TemplateName) VALUES ('"
                        ...
                        for (int i = 0; i < Gridview1.Rows.Count; i++)
                        {
                            ...
                            if (box3.Checked == true)
                                ...
                                check = 0;
                            StrQuery = @"INSERT INTO ConceptMap_TemplateLinkData(TemplateID, FieldName, FieldValue, Type, Show) VALUES ("
                                + myID + ", '"
                                + box1.Text + "', '"
                                + box2.Text + "', 'General',"
                                + check + ");";
        ...
        protected void bindTemplate()
        {
            ...
            strcon1.Open();
            string ADDStr = "SELECT TemplateID, TemplateName FROM ConceptMap_LinkTemplate";
        ...
        protected void OnRowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                SqlConnection strcon1 = new SqlConnection(strConnString);
                strcon1.Open();
                string ADDStr = "SELECT TemplateNodeID, TemplateID, FieldName, FieldValue FROM ConceptMap_TemplateLinkData where Type='General' order by FieldName";
                SqlCommand ADDCmd = new SqlCommand(ADDStr, strcon1);
                DataTable table = new DataTable();
        ...
        protected void OnRowDeleting(object sender, GridViewDeleteEventArgs e)
        {
            ...
            DataTable dt = ViewState["CurrentLinkTable"] as DataTable;
        ...
        protected void OnRowDataBoundSpecific(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                SqlConnection strcon1 = new SqlConnection(strConnString);
                strcon1.Open();
                string ADDStr = "SELECT TemplateNodeID, TemplateID, FieldName, FieldValue FROM ConceptMap_TemplateLinkData where Type='Specific' order by FieldName";
                SqlCommand ADDCmd = new SqlCommand(ADDStr, strcon1);
                DataTable table = new DataTable();
        ...
        {
            int index = Convert.ToInt32(e.RowIndex);
            DataTable dt = ViewState["CurrentLinkTable2"] as DataTable;
            dt.Rows[index].Delete();
            ViewState["CurrentLinkTable2"] = dt;
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                dt.Rows[i]["RowNumber"] = i + 1;
            }
        ...
        protected void DropDownList1_SelectedIndexChanged(object sender, EventArgs e)
        {
            using (SqlConnection Cn = new SqlConnection(strConnString))
            {
                using (SqlCommand Cmd = new SqlCommand("select Type, FieldName,FieldValue from ConceptMap_TemplateLinkData where TemplateID = @templateid order by Type, FieldName", Cn))
                {
                    Cn.Open();
        ...
        private void SetDropDown1()
        {
            using (SqlConnection Cn = new SqlConnection(strConnString))
            {
                using (SqlCommand Cmd = new SqlCommand("SELECT Type, FieldName, FieldValue FROM ConceptMap_TemplateLinkData WHERE (TemplateID = (SELECT     TOP (1) TemplateID FROM ConceptMap_TemplateLinkData AS ConceptMap_TemplateLinkData_1 ORDER BY TemplateID)) ORDER BY Type, FieldName", Cn))
                {
                    Cn.Open();
        ...
        private void BindGridView()
        {
            if (ViewState["CurrentLinkTable"] != null)
            {
                Gridview1.DataSource = ViewState["CurrentLinkTable"];
                Gridview1.DataBind();
            }
            else
            {
        ...
        protected void GenerateControls()
        {
            ...
            while (myDataReader.Read())
            {
                strQuery = "select FieldName, FieldValue, Type" +
                                   ...
                                   "where " +
                                   "ConceptMap_TemplateLinkData.TemplateID=" + myDataReader["TemplateID"].ToString();
        ...
        protected void ButtonSelectNodeTemplate_Click(object sender, EventArgs e)
        {
            ...
            //foreach (var control in accordion.Controls)
            {
                ...
                if (((CheckBox)chk).Checked)
                {
                    ...
                    _nodetemplate.Clients.All.addLinkItems(chkValue.ToString(), txt.Text, chk.Text);
                    _nodetemplate.Clients.All.setTemplateData("linktemplate-" + chkValue.ToString(), DataTableToJSON((DataTable)grd.DataSource));
            ...
            //    Application["linkTemplate"] = nTemplate;
            ...
            if (hfNodeValue.Value != "")
            {
                ...
                string StrQuery;
                StrQuery = @"Delete From ConceptMap_SelectedLinkTemplate Where ORCSGroupID = '" + strNodeID + "'";
                execquery(StrQuery);
                //迴圈執行檢查被勾選Template的內容
                foreach (NodeTemplate varTemplate in nTemplate)
                {
                    StrQuery = @"INSERT INTO ConceptMap_SelectedLinkTemplate(ORCSGroupID, TemplateID, TemplateColor) VALUES ('"
